...
const getAllBookingsForOwner = (owner_id) => {
  return pool
  .query(
    `SELECT bookings.booking_id, rides.ride_id, users.name, users.avatar, users.contact, bookings.rider_id, bookings.seats_booked, bookings.booking_status FROM bookings JOIN rides ON 
